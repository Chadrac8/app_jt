...
void main() async {
  print('🔄 SCRIPT DE MIGRATION PEOPLE → PERSONS');
  print('=' * 50);
...
2. 🔍 ANALYSE DES DONNÉES
   ...
   - Détecter les doublons par email/nom
   - Comparer les champs et versions des données

3. 🚀 MIGRATION SÉCURISÉE
   - Copier les données uniques de 'people' vers 'persons'
   - Fusionner les informations des doublons (garder la plus récente)
   - Préserver les relations (roles, familyId, etc.)
   - Maintenir les timestamps et métadonnées

4. ✅ VALIDATION
   - Vérifier l'intégrité des données migrées
   - Confirmer que tous les enregistrements sont présents
...
- La migration peut prendre plusieurs minutes selon le volume de données
- L'application doit être en mode maintenance pendant la migration

🔒 SÉCURITÉ:
- Toutes les opérations sont effectuées en transactions atomiques
- Rollback automatique en cas d'erreur
- Logs détaillés de toutes les opérations
- Validation des données à chaque étape
  ''');
  
  print('\n❓ ÊTES-VOUS PRÊT À LANCER LA MIGRATION?');
  print('Cette opération va :');
  print('- Migrer toutes les données de "people" vers "persons"');
  print('- Créer une sauvegarde de sécurité');
  print('- Résoudre les doublons automatiquement');
  print('\nTapez "MIGRER" pour confirmer (en majuscules):');
  
  final input = stdin.readLineSync();
  if (input == 'MIGRER') {
    print('\n🚀 MIGRATION CONFIRMÉE! Génération du script...');
    await generateMigrationScript();
  } else {
    print('\n⏸️  Migration annulée. Tapez exactement "MIGRER" pour confirmer.');
    exit(1);
  }
}
...
''';
  ...
  print('✅ Script de migration généré: $scriptPath');
  print('\n🔧 INSTRUCTIONS:');
  print('1. Vérifiez le script généré');
  print('2. Lancez-le avec: dart run $scriptPath');
  print('3. Surveillez les logs pendant l\'exécution');
  print('4. Validez les résultats avant de continuer');
}