...
class _BiblePageState extends State<BiblePage> with SingleTickerProviderStateMixin {
  ...
  Widget _buildModernNotesHeader(int allCount, int notesCount, int highlightsCount) {
    return Container(
      margin: const EdgeInsets.all(20),
      decoration: BoxDecoration(
        gradient: LinearGradient(
                ...
                Expanded(
                  child: Column(
                    ...
                    children: [
                      ...
                      Text(
                        allCount > 0 
                            ? 'Journal personnel d\'étude biblique'
                            : 'Votre espace d\'étude personnel',
                        style: GoogleFonts.inter(
                          fontSize: 14,
                          color: Colors.white.withOpacity(0.9),
                          fontWeight: FontWeight.w500,
                ...
                children: [
                  ...
                  Expanded(
                    child: _buildStatsCard(
                      ...
                      label: 'Surligné${highlightsCount > 1 ? 's' : ''}',
                      color: Colors.orange,
                    ),
                  ),
                  const SizedBox(width: 12),
  ...
  Widget _buildModernFilterSelector(String selectedFilter, Function(String) onFilterChanged) {
    final filters = [
      {
        'id': 'all',
        'label': 'Tout',
  ...
  Widget _buildNotesEmptyState(String selectedFilter) {
    final config = _getEmptyStateConfig(selectedFilter);
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 32),
            _buildQuickActionCards(selectedFilter),
          ],
        ),
      ),
    );
  ...
  Map<String, dynamic> _getEmptyStateConfig(String filter) {
    switch (filter) {
      case 'notes':
        return {
          'icon': Icons.sticky_note_2,
          'title': 'Aucune note personnelle',
          'subtitle': 'Commencez à prendre des notes\nsur vos versets préférés',
          'color': Colors.green,
        };
      case 'highlights':
        return {
          'icon': Icons.highlight,
          'title': 'Aucun verset surligné',
          'subtitle': 'Surlignez les passages\nqui vous marquent',
          'color': Colors.orange,
        };
      default:
        return {
          ...
          'subtitle': 'Prenez des notes et surlignez\npour enrichir votre étude',
          'color': Colors.purple,
        };
    }
  }

  Widget _buildQuickActionCards(String filter) {
    if (filter != 'all') {
      return Container(
        padding: const EdgeInsets.all(20),
        decoration: BoxDecoration(
        ...
        child: Column(
          children: [
            ...
            Text(
              filter == 'notes' 
                  ? 'Appuyez longuement sur un verset\npour ajouter une note'
                  : 'Sélectionnez un verset puis\nappuyez sur l\'icône de surlignage',
              style: GoogleFonts.inter(
                fontSize: 14,
                color: Colors.purple[600],
                height: 1.4,
  ...
  Widget _buildModernNotesList(List<BibleVerse> verses, Function setModalState) {
    return CustomScrollView(
      physics: const BouncingScrollPhysics(),
      slivers: [
        SliverPadding(
          ...
          sliver: SliverList(
            delegate: SliverChildBuilderDelegate(
              (context, index) {
                ...
                return Container(
                  margin: const EdgeInsets.only(bottom: 16),
                  child: _buildModernNoteCard(verse, index, setModalState),
                );
              },
  ...
  Widget _buildModernNoteCard(BibleVerse verse, int index, Function setModalState) {
    final key = _verseKey(verse);
    final hasNote = _notes.containsKey(key) && _notes[key]!.isNotEmpty;
    final hasHighlight = _highlights.contains(key);
    final note = _notes[key];
      ...
      child: Container(
        decoration: BoxDecoration(
          gradient: LinearGradient(
            ...
            colors: [
              Colors.white,
              hasHighlight ? Colors.purple[25]! : Colors.grey[25]!,
            ],
          ),
          borderRadius: BorderRadius.circular(20),
          border: Border.all(
            color: hasHighlight ? Colors.purple.withOpacity(0.3) : Colors.grey.withOpacity(0.2),
          ),
          boxShadow: [
            BoxShadow(
              color: (hasHighlight ? Colors.purple : Colors.grey).withOpacity(0.1),
              blurRadius: 12,
              offset: const Offset(0, 4),
            ),
          ],
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              ...
              // Texte du verset avec style biblique
              Container(
                padding: const EdgeInsets.all(20),
                decoration: BoxDecoration(
                  color: Colors.white,
              ...
              Container(
                ...
                child: Row(
                  ...
                  children: [
                    _buildNoteAction(
                      icon: hasHighlight ? Icons.highlight_off : Icons.highlight,
                      label: hasHighlight ? 'Surligné' : 'Surligner',
                      color: hasHighlight ? Colors.orange[600]! : Colors.grey[500]!,
                    ...
                    _buildNoteAction(
                      ...
                      label: hasNote ? 'Éditer' : 'Noter',
                      color: hasNote ? Colors.green[600]! : Colors.grey[500]!,
                      isActive: hasNote,
                      onTap: () => _editNoteDialog(verse),
                    ),
                    _buildNoteAction(
                      ...
                      color: isFavorite ? Colors.amber[600]! : Colors.grey[500]!,
                      isActive: isFavorite,
                      onTap: () {
                        HapticFeedback.lightImpact();
