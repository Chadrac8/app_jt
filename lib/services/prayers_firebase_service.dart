...
class PrayersFirebaseService {
  ...
  // Stream des prières avec filtres (optimisé pour éviter les erreurs d'index)
  static Stream<List<PrayerModel>> getPrayersStream({
    ...
    try {
      ...
      return query.snapshots().map((snapshot) {
        ...
        if (limit > 0 && prayers.length > limit) {
          prayers = prayers.take(limit).toList();
        }
  ...
  // Obtenir les catégories de prières disponibles
  static Future<List<String>> getCategories() async {
    try {
      ...
      final categories = <String>{};
      for (final doc in snapshot.docs) {
        final prayer = PrayerModel.fromMap(doc.data() as Map<String, dynamic>);
        if (prayer.category.isNotEmpty) {
          categories.add(prayer.category);
        }
      }
  ...
  // Stream simplifié pour éviter les erreurs d'index
