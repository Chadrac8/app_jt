...
class AppConfigFirebaseService {
  ...
  static const String configDocumentId = 'main_config_v2';
  ...
  static Future<void> updateModuleConfig(String moduleId, {
    ...
    bool moduleExists = config.modules.any((module) => module.id == moduleId);
    ...
    if (moduleExists) {
      ...
      final newModule = ModuleConfig(
        ...
        isPrimaryInBottomNav: isPrimaryInBottomNav ?? false,
        order: order ?? 0,
        isBuiltIn: true,
        coverImageUrl: coverImageUrl,
        showCoverImage: showCoverImage ?? false,
        coverImageHeight: coverImageHeight ?? 200.0,
      );
  ...
  static Future<void> initializeDefaultConfig() async {
    ...
    if (!doc.exists) {
      ...
      // Mettre à jour la configuration existante avec les nouveaux modules
      await _updateConfigWithNewModules();
    }
  }

  // Mettre à jour la configuration existante avec les nouveaux modules
  static Future<void> _updateConfigWithNewModules() async {
    try {
      final config = await getAppConfig();
      final defaultModules = _getDefaultModules();
      final existingModuleIds = config.modules.map((m) => m.id).toSet();
      
      // Trouver les nouveaux modules qui n'existent pas encore
      final newModules = defaultModules.where((m) => !existingModuleIds.contains(m.id)).toList();
      
      if (newModules.isNotEmpty) {
        print('🔄 Ajout de ${newModules.length} nouveaux modules: ${newModules.map((m) => m.name).join(', ')}');
        
        final updatedModules = [...config.modules, ...newModules];
        ...
        await updateAppConfig(updatedConfig);
        print('✅ Nouveaux modules ajoutés avec succès');
      }
    } catch (e) {
  ...
  static List<ModuleConfig> _getDefaultModules() {
    return [
      ...
      ModuleConfig(
        id: 'bible',
        name: 'La Bible',
        description: 'Lire et rechercher dans la Bible',
        iconName: 'bible',
        route: 'bible',
        category: 'core',
        isEnabledForMembers: true,
      ...
      ModuleConfig(
        id: 'songs',
        name: 'Cantiques',
        description: 'Collection de chants et cantiques pour l\'église',
        iconName: 'music_note',
        route: 'songs',
        category: 'ministry',
        isEnabledForMembers: true,
