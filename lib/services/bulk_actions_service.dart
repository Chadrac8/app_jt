...
class BulkActionsService {
  ...
  Future<BulkActionResult> deletePersons({
    required List<PersonModel> people,
  }) async {
    try {
      final results = <String, bool>{};
      final errors = <String>[];
      final personIds = people.map((p) => p.id).toList();
      try {
        ...
        for (final id in personIds) {
          results[id] = false;
          errors.add('Erreur suppression $id: $e');
        }
      }
      await _recordBulkAction(
        type: BulkActionType.statusUpdate, // ou créer BulkActionType.delete si besoin
        peopleIds: personIds,
  ...
  /// Envoyer un message réel (email ou notification) à plusieurs personnes
  Future<BulkActionResult> sendMessage({
    required List<PersonModel> people,
    required String message,
  }) async {
    // À adapter selon votre infrastructure d'envoi (API, SMTP, FCM, etc.)
    // Ici, on simule l'envoi avec un succès pour chaque personne
    final results = <String, bool>{};
    final errors = <String>[];
    ...
    await _recordBulkAction(
      type: BulkActionType.email, // ou sms, notification selon le canal
