...
class _GroupMemberAttendanceStatsState extends State<GroupMemberAttendanceStats>
  ...
  List<MemberAttendance> get _filteredAndSortedMembers {
    List<MemberAttendance> members = widget.memberAttendance.values.toList();
    // Filter
    switch (_filterBy) {
      case 'excellent':
  ...
  Widget _buildMemberCard(MemberAttendance member) {
    // Sécurisation des données du membre
    ...
    return Container(
      ...
      child: ExpansionTile(
        leading: CircleAvatar(
          backgroundColor: _getAttendanceColor(member.attendanceRate).withOpacity(0.2),
          child: Text(
  ...
  Widget _buildProgressBar(MemberAttendance member) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        Text(
  ...
  Color _getAttendanceColor(double attendanceRate) {
