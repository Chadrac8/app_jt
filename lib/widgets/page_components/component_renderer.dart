...
class ComponentRenderer extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    switch (component.type) {
      ...
      case 'tabs':
        return _buildTabsComponent(context);
  ...
  Widget _buildTabsComponent(BuildContext context) {
              ...
              child: const Center(
                child: Column(
                  ...
                  children: [
                    ...
                    Text('(nécessite Google Maps)', style: TextStyle(fontSize: AppTheme.fontSize12)),
                  ],
                ),
              ),
            ),
  ...
  Widget _buildAudioComponent() {
    ...
    // Gestion des albums multi-sources
    if (sourceType == 'album') {
      final albumUrl = component.data['album_url'] ?? '';
      final albumFiles = component.data['album_files'] as List<dynamic>? ?? [];
      List<Widget> tracks = [];

      // Ajout des fichiers locaux sélectionnés
      for (var file in albumFiles) {
        tracks.add(MediaPlayerService.buildAudioFilePlayer(
          url: file['path'] ?? '',
          title: file['name'] ?? '',
          autoPlay: false,
        ));
      ...
      // Ajout d'un lien album (SoundCloud, Drive, OneDrive)
      if (albumUrl.isNotEmpty) {
        if (albumUrl.contains('soundcloud.com')) {
          tracks.add(MediaPlayerService.buildSoundCloudPlayer(
            url: albumUrl,
            autoPlay: false,
            showComments: showComments,
            color: color,
          ));
        } else if (albumUrl.contains('drive.google.com') || albumUrl.contains('onedrive.live.com')) {
          tracks.add(MediaPlayerService.buildAudioFilePlayer(
            url: albumUrl,
            title: 'Album Cloud',
            autoPlay: false,
      ...
      return Builder(
        builder: (context) => Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            Text('Album audio', style: Theme.of(context).textTheme.titleMedium),
            const SizedBox(height: AppTheme.spaceSmall),
            ...tracks,
          ],
        ),
              ...
              child: const Center(
                child: Column(
                  ...
                  children: [
                    ...
                    SizedBox(height: AppTheme.spaceSmall),
                    Text('Google Maps'),
                    Text('(Intégration nécessaire)', style: TextStyle(fontSize: AppTheme.fontSize12)),
                  ],
                ),
              ),
            ),
              ...
              if (allowDirectJoin && group['isJoinable']) ...[
                SizedBox(
                  ...
                  child: ElevatedButton(
                    ...
                    child: const Text('Rejoindre', style: TextStyle(fontSize: AppTheme.fontSize12)),
                  ),
                ),
              ],
            ],
  ...
  Widget _buildGridContainerComponent(BuildContext context) {
    ...
    final autoHeight = component.data['autoHeight'] ?? true;
    final maxHeight = (component.data['maxHeight'] ?? 400.0).toDouble();

    // Styling
    final backgroundColor = Color(int.parse(
      component.styling['backgroundColor']?.replaceAll('#', '0xFF') ?? '0xFFFFFFFF'
    ));
    final borderColor = Color(int.parse(
      component.styling['borderColor']?.replaceAll('#', '0xFF') ?? '0xFFE0E0E0'
    ));
    final borderWidth = (component.styling['borderWidth'] ?? 1.0).toDouble();
    final borderRadius = (component.styling['borderRadius'] ?? 8.0).toDouble();
    final elevation = (component.styling['elevation'] ?? 0.0).toDouble();

    // Filtrer les composants enfants visibles
    final visibleChildren = component.children.where((child) {
      return true; // Pour l'instant, on affiche tous les enfants
    }).toList();
          ...
          children: [
            ...
            if (allowRegistration && event['registrationOpen'])
              ElevatedButton(
                ...
                child: const Text('S\'inscrire', style: TextStyle(fontSize: AppTheme.fontSize12)),
              ),
          ],
        ),
      )).toList(),
  ...
  Widget _buildPrayerWallComponent(BuildContext context) {
    final title = component.data['title'] ?? 'Prières & Témoignages';
            ...
            children: [
              ...
              FloatingActionButton.extended(
                onPressed: () => _showPrayerSubmissionDialog(
                  context, allowRequests, allowTestimonies, 
              ...
              Row(
                children: [
                  ...
                  if (allowPrayerCount) ...[
                    ...
                    const SizedBox(width: AppTheme.spaceXSmall),
                    Text('${prayer['prayerCount']}', style: TextStyle(fontSize: AppTheme.fontSize12, color: primaryColor)),
                  ],
                ],
              ),
              const SizedBox(height: AppTheme.spaceXSmall),
                ...
                Row(
                  children: [
                    if (showAuthors) ...[
                      Text(
                        prayer['author'],
                        style: TextStyle(fontSize: AppTheme.fontSize11, color: AppTheme.grey500),
                      ),
                      if (showDates) const Text(' • ', style: TextStyle(fontSize: AppTheme.fontSize11)),
                    ],
                    if (showDates)
                      Text(
            ...
            child: Column(
              ...
              children: [
                ...
                Container(
                  ...
                  child: Text(
                    _errorMessage,
                    style: TextStyle(color: AppTheme.grey600, fontSize: AppTheme.fontSize14),
                    textAlign: TextAlign.center,
                  ),
                ),
                const SizedBox(height: AppTheme.space20),
                  ...
                  child: Row(
                    ...
                    children: [
                      ...
                      Flexible(
                        child: Text(
                          'URL testée: $url',
                          style: TextStyle(color: AppTheme.grey700, fontSize: AppTheme.fontSize12),
                          overflow: TextOverflow.ellipsis,
                        ),
                      ),
                    ],
        ...
        child: Center(
          child: Column(
            ...
            children: [
              ...
              Text(
                'URL non configurée',
                style: TextStyle(color: AppTheme.grey600, fontSize: AppTheme.fontSize16),
              ),
              const SizedBox(height: AppTheme.spaceXSmall),
              Text(
                'Veuillez modifier ce composant pour ajouter une URL',
                style: TextStyle(color: AppTheme.grey500, fontSize: AppTheme.fontSize14),
                textAlign: TextAlign.center,
              ),
            ],
          ),
          ...
          child: Column(
            ...
            children: [
              ...
              Text(
                ...
                style: const TextStyle(color: AppTheme.grey500, fontSize: AppTheme.fontSize16),
              ),
              const SizedBox(height: AppTheme.spaceSmall),
              Container(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      Text(
                        ...
                        style: TextStyle(color: AppTheme.grey500, fontSize: AppTheme.fontSize16),
                      ),
                    ],
                  ),
                ),
